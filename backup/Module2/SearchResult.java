...
public class SearchResult {
    ...
    List<List<String>> expectedTableBody, WebDriver driver) {
        ...
        try {
            ...
            for(int i=0;i<expectedTableHeaders.size();i++){
                ...
                String actuaHeaderlText=tableHeaderElement.getText();            
                if(!actuaHeaderlText.equals(expectedHeaderText)){
                    status = false;
                }
            ...
            for(int i=0;i<expectedTableBody.size();i++){
              ...
              List <String> exptectedColumns=expectedTableBody.get(i);

              for(int j=0;j<exptectedColumns.size();j++){
                String expectedColumText=exptectedColumns.get(j);    
                int row=i+1;
                int column=j+1;

                WebElement actualTableBody=driver.findElement(By.xpath("//table/tbody/tr[" + row + "]/td[" + column + "]"));
                String actualColumnText=actualTableBody.getText();                          
                if(!actualColumnText.equals(expectedColumText)){
                    status = false;
                }
    ...
    public Boolean verifyExistenceofSizeDropdown(WebDriver driver) {
        ...
        try {
            ...
            WebElement sizeDropdownElement=driver.findElement(By.xpath("//select[@name='age']"));
            if(sizeDropdownElement.isDisplayed()){
                status = true;
            }
            return status;
        } catch (Exception e) {
